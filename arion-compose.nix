{ pkgs, ... }: {
  config.project.name = "haskell-web-api-template";
  config.services = {
    ...
    haskell-web-api-template-service = {
      image.enableRecommendedContents = true;
      service.useHostStore = true;
      ...
      service.defaultExec = [
        "sh"
        "-c"
        "${pkgs.haskell-web-api-template.haskell-web-api-template-exe.outPath}/bin/haskell-web-api-template --pageUrl https://en.swissquote.com/forex/pricing/trading-conditions --uploadUrl http://postgrest-service:3000/api/updateSQTradingCondition"
      ];
      service.ports = [ ];
      service.environment.ENV1 =
        "${pkgs.haskell-web-api-template.haskell-web-api-template-exe.outPath}/share/doc";
